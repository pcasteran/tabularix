...
Verify Vertical Table Extraction Header And Data
    ...
    ${header}=    Evaluate
    ...    tabularix.group(tabularix.value("Region"), tabularix.regex(r"^Q[1-4]$").repeat(4, max=4))
    ...    modules=tabularix
    ${data_expr}=    Catenate
    ...    tabularix.grid(
    ...    tabularix.group(
    ...    tabularix.regex(r"^(?!Total).*$"),
    ...    tabularix.non_empty().repeat(4, max=4)
    ...    ).one_or_more()
    ...    )
    ${data}=    Evaluate    ${data_expr}    modules=tabularix
...
Verify Vertical Table Extraction Between Header and Footer
    ...
    ...    modules=tabularix
    ${footer}=    Evaluate
    ...    tabularix.group(tabularix.regex(r"^Total \\d{4}$"), tabularix.any().repeat(4, max=4))
    ...    modules=tabularix
    ${table}=    Evaluate
    ...    tabularix.extract_table_between_header_and_footer($sheet, $header, $footer)
    ...    modules=tabularix
    ${cols}=    Evaluate    $table.columns
    Should Be Equal As Strings    ${cols}    ['region', 'q1', 'q2', 'q3', 'q4']
    ${shape}=    Evaluate    $table.shape
...
Verify Horizontal Table Extraction Header And Data
    ...
    ${header}=    Evaluate
    ...    tabularix.group(tabularix.regex(r"^(Date|Fiscal Year)$").repeat(2, 2))
    ...    modules=tabularix
    ...
    ...    $sheet, $header, $data,
    ...    main_direction="LR", inner_direction="TB"
    ...    )
    ${table}=    Evaluate    ${table_expr}    modules=tabularix
    ${cols}=    Evaluate    $table.columns
