...
Verify Cell Cardinality Repeat Custom Range Interval
    ...
    ${matcher2}=    Evaluate    tabularix.RangeMatcher().row($pattern2)    modules=tabularix
...
Verify Row Cardinality Repeating
    ...
    ${matcher}=    Evaluate    tabularix.RangeMatcher().row($pattern).one_or_more()    modules=tabularix
