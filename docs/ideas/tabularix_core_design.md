...
2. **RangeMatcher API (Layex)**: Declaratively search for structural ranges using either a String DSL or a Python Object Builder.
...
- [ ] **RangeMatcher Ergonomics**: The Layex engine can elegantly handle complex cardinality (e.g., nested groups of cells `([type:string] [type:numeric]+)*`).
...
- **Table Assembly**: Construct an Apache Arrow table from discrete extracted blocks: `build_table_from_ranges(header, data, footer)`.
